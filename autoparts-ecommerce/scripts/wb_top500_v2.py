"""
wb_top500_v2.py — КОМБИНИРОВАННЫЙ АНАЛИЗ: Топ-500 Автолиги для WB
=================================================================
Источники данных:
  1. Автолига прайс          PriceALVLG0411.xls (26.05.2026, 41 143 арт.)
  2. MPStats кэш             683 бренда, 52 695 авто-товаров WB
  3. Drive2 / Drom           Топ-15 авто РФ × частота замены деталей
  4. Автостат 2025           Объём автопарка по моделям
  5. WB-аналитика 2024-2026  Рост категорий: масла +313%, подвеска +155%
  6. Шопстат / MPStats блог  Топ ниш и категорий (из открытых отчётов)

8 сигналов в одном скоре:
  S1  Дефицит WB          OOS конкурентов по бренду (MPStats)
  S2  Голубой океан        Мало товаров бренда на WB
  S3  Автопарк × поломка   fleet_size × repair_frequency (Drive2 + Автостат)
  S4  Расходник            Высокая частота замены (< 30 000 км)
  S5  Сезон лето 2026      Категории с пиком летом
  S6  Уникальность WB      Бренд не представлен или слабо
  S7  Ценовой мультипликатор  Высокая наценка при закупке
  S8  Широкая применяемость   Деталь подходит ко многим авто
"""
...
OUTPUT_FILE = "data/analytics/wb_top500_v2.xlsx"
...
WB_RETURN_RATE = 0.03
TAX_RATE       = 0.06
WB_LOGISTICS   = 75      # ₽ FBS средний (немного выше из-за упаковки)
MIN_MARGIN_PCT = 15
TARGET_MARGIN  = 25
MAX_PER_BRAND  = 15      # лимит товаров на бренд в топ-500

# ══════════════════════════════════════════════════════════════════════════════
#  ДАННЫЕ ИЗ ИССЛЕДОВАНИЙ (HARDCODED)
# ══════════════════════════════════════════════════════════════════════════════

# ── Автопарк РФ (Автостат 2025, тыс. шт.) ────────────────────────────────────
CAR_FLEET = {
    # LADA (26.9% парка)
    ...
    # Китайцы (быстрый рост 2022-2026)
...
# ── Частота замены деталей (Drive2/Drom, раз на 100 000 км) ──────────────────
# Чем выше → тем чаще нужно → тем стабильнее спрос
PART_FREQ = {
    # Расходники (очень часто)
    "фильтр масл":      6.0,   # каждые 10-15к км
    "масляный фильтр":  6.0,
    "фильтр воздуш":    3.5,   # каждые 15-20к
    "воздушный фильтр": 3.5,
    "фильтр салон":     3.5,
    "салонный фильтр":  3.5,
    "свеч":             2.5,   # каждые 30-40к
    "spark plug":       2.5,
    "масло мотор":      6.0,
    "моторное масло":   6.0,
    "антифриз":         1.5,   # каждые 60-80к
    "охлаждающ жидк":   1.5,
    "жидкость тормозн": 1.5,
    # Тормоза (часто)
    "тормозн колодк":   2.5,   # каждые 30-50к
    "колодки тормозн":  2.5,
    "brake pad":        2.5,
    "тормозной диск":   1.5,
    # Подвеска (часто)
    "амортизатор":      1.5,   # каждые 50-100к
    "стойк стабил":     2.0,   # каждые 30-60к
    "стабилизатор":     1.5,
    "сайлентблок":      2.0,   # каждые 30-80к
    "шаровая":          1.5,
    ...
    # Привод/трансмиссия
    "шрус":             1.2,
    "ремень грм":       1.0,   # каждые 60-100к
    "ремень привода":   1.5,   # каждые 50-70к
    "ремень генератор": 1.5,
    "натяжитель":       1.0,
    "цепь грм":         0.8,
    # Рулевое
    "рулевая рейка":    0.8,   # каждые 100-150к
    "рулевой наконечн": 1.2,
    # Охлаждение
    "помпа":            1.0,   # каждые 80-120к
    "водяной насос":    1.0,
    "радиатор":         0.8,
    # Электрика
    "катушка зажиган":  1.2,
    "генератор":        0.6,
    "стартер":          0.5,
    "дворники":         2.0,   # раз в год минимум
    "щетки стеклооч":   2.0,
    # Двигатель
    "прокладка гбц":    0.5,
    "форсунк":          1.0,
    "термостат":        0.8,
    # Кузов/расходники
    "аккумулятор":      0.7,   # каждые 3-5 лет
    "лампа":            2.0,   # по мере перегорания
    "лампочка":         2.0,
}

# ── Сезон лето 2026 (пик продаж июнь-август) ─────────────────────────────────
# WB-статистика: масла +313%, подвеска +155%, шины +274%, антифриз +180%
SUMMER_KW = [
...
# ── Универсальные детали (широкая применяемость → выше спрос) ────────────────
WIDE_APPLY_KW = [
    "универсальн", "подходит для всех", "все авто",
    "масло", "антифриз", "тормозная жидк", "омыватель",
    "w", "5w", "10w", "15w",  # вязкость масла → универсально
]

# ── Категории WB (фильтрация MPStats) ────────────────────────────────────────
AUTO_KEYS = [
...
# ── Топ-ниши WB автозапчасти (из открытых отчётов MPStats/ShopStat 2025) ──────
# Объёмы выручки (ориентировочно, млн ₽/мес)
WB_TOP_NICHES = {
...
def get_fleet_score(name: str) -> float:
    """Взвешенный балл автопарка: sum(fleet_size) для всех моделей в названии."""
    t = name.lower()
    total = 0
    matched = []
    ...
    return min(total / 500, 3.0), matched  # нормализация 0—3

def get_repair_freq(name: str) -> float:
    """Частота замены детали (из Drive2 данных)."""
    t = name.lower()
    best = 0.0
    matched_part = ""
    for kw, freq in PART_FREQ.items():
        if kw in t and freq > best:
            best = freq
            matched_part = kw
    return min(best / 3.0, 2.0), matched_part  # нормализация 0—2

def get_niche_score(name: str) -> tuple:
    """Соответствие топовым WB-нишам."""
    t = name.lower()
    best_score = 0.0
    best_niche = ""
...
def score_item(item: dict, brand_stats: dict) -> dict:
    ...
    # ── S3: Автопарк × частота поломки (Drive2 + Автостат) ───────────────────
    fleet_score, fleet_cars  = get_fleet_score(name_lo)
    repair_score, repair_part = get_repair_freq(name_lo)
    s3 = 1.0 + fleet_score * 0.3 + repair_score * 0.3   # 1.0–2.8

    # ── S4: Расходник (сверхчастая замена) ───────────────────────────────────
    freq_raw, _ = get_repair_freq(name_lo)
    s4 = 1.0 + min(freq_raw / 2.0, 0.5)                 # 1.0–1.5
    ...
    # ── S7: WB топ-ниша (из ShopStat/MPStats отчётов) ────────────────────────
    niche_score, niche_name = get_niche_score(name_lo)
    s7 = 1.0 + niche_score * 0.4                         # 1.0–1.8

    # ── S8: Широкая применяемость ────────────────────────────────────────────
    ...
    if repair_part:   parts.append(f"Деталь меняют: {repair_part} (×{freq_raw:.1f}/100к км)")
...
def export(results: list) -> None:
    ...
    sig_cols = ["#","Артикул","Бренд","Score","S1 Дефицит","S2 Океан",
    ...
    for i, it in enumerate(results[:50], 2):
        row2 = [i-1, it['article'], it['brand'], it['score'],
                it.get('s1'), it.get('s2'), it.get('s3'), it.get('s4'),
                it.get('s5'), it.get('s6'), it.get('s7'), it.get('s8'),
                it.get('mg_pct')]
        for c, val in enumerate(row2, 1):
            _cell(ws2, i, c, val, align="center", fmt=('0.00' if c>=4 else None))

    # ── Лист 3: Стратегии ─────────────────────────────────────────────────────
    ws3 = wb.create_sheet("Стратегии")
    cnt3 = defaultdict(int)
    for it in results[:500]:
        for s in it.get('strats','').split(', '):
            if s.strip(): cnt3[s.strip()] += 1
    ws3.append(["Стратегия", "Товаров в Топ-500", "% от топ-500"])
    for s, n in sorted(cnt3.items(), key=lambda x: -x[1]):
        ws3.append([s, n, round(n/5, 1)])
    ws3.column_dimensions['A'].width = 40
    ws3.column_dimensions['B'].width = 20
    ws3.column_dimensions['C'].width = 15

    # ── Лист 4: Топ ниши ─────────────────────────────────────────────────────
    ws4 = wb.create_sheet("WB Топ-ниши")
    ws4.append(["Ниша", "Выручка WB млн₽/мес*", "Рост 2024", "Продавцов", "В нашем топ-500"])
    ...
    for niche, data in sorted(WB_TOP_NICHES.items(), key=lambda x: -x[1]['revenue_est']):
        ws4.append([niche, data['revenue_est'], f"×{data['growth']:.2f}",
                    data['sellers'], niche_cnt.get(niche, 0)])
    ws4.column_dimensions['A'].width = 30
    for col in ['B','C','D','E']: ws4.column_dimensions[col].width = 18
    ws4.append([])
    ws4.append(["* Оценочные данные на основе открытых отчётов MPStats/ShopStat 2025"])

    # ── Лист 5: Топ брендов ───────────────────────────────────────────────────
    ws5 = wb.create_sheet("Бренды")
    bd = defaultdict(lambda:{'cnt':0,'sales':0,'oos':0,'mg':[]})
    for it in results[:500]:
        b = it['brand']
        bd[b]['cnt']   += 1
    ...
    ws5.append(["Бренд","В топ-500","WB продажи шт/мес","OOS %","Ср.маржа %"])
    for b, d in sorted(bd.items(), key=lambda x: -x[1]['cnt']):
        ...
        ws5.append([b, d['cnt'], round(d['sales']), round(avg_oos,1), round(avg_mg,1)])
    for col in ['A','B','C','D','E']: ws5.column_dimensions[col].width = 20

    # ── Лист 6: Методология ───────────────────────────────────────────────────
    ws6 = wb.create_sheet("Методология")
    notes = [
        ("ИСТОЧНИКИ ДАННЫХ", ""),
        ("Автолига прайс", "PriceALVLG0411.xls, 26.05.2026, 41 143 арт. со стоком"),
        ("MPStats кэш", "683 бренда, 52 695 авто-товаров WB (период апр-май 2026)"),
        ...
        ("Возвраты", "3%"),
        ("УСН", "6% от выручки"),
        ("Логистика", "75 руб. FBS Волгоград (≈0.3 кг)"),
        ("Мин. маржа", "≥ 15% (иначе товар отброшен)"),
        ("Цель. маржа", "25% — рекомендованная цена продажи"),
        ("", ""),
        ("ФОРМУЛА СКОРА (8 СИГНАЛОВ)", ""),
        ("S1 Дефицит",       "1 + avg_oos/100 × 2  →  1.0—3.0"),
        ("S2 Голубой океан", "зависит от кол-ва WB-прод. бренда (0→1.8, <10→1.4, <30→1.2)"),
        ("S3 Автопарк×поломка","1 + fleet_score×0.3 + repair_score×0.3  →  1.0—2.8"),
        ("  fleet_score",    "Σ автопарка(тыс.)/500, нормировано 0—3"),
        ("  repair_score",   "частота замены (из Drive2) / 3, нормировано 0—2"),
        ("S4 Расходник",     "1 + min(freq/2, 0.5)  →  1.0—1.5"),
        ("S5 Сезон лето",    "×1.25 для летних категорий (масла, антифриз, тормоза, кондиц.)"),
        ("S6 Уникальность",  "×1.5 нет на WB; ×1.2 мало (<5 WB-прод.)"),
        ("S7 WB-ниша",       "1 + niche_score×0.4  (log выручка × рост / √продавцов)"),
        ("S8 Широкий",       "×1.15 для универсальных товаров (масла, антифриз, etc.)"),
        ("Score итог",       "demand × S1 × S2 × margin_score × S3 × S4 × S5 × S6 × S7 × S8"),
        ("", ""),
        ("ВАЖНО", "Матчинг по артикулу недоступен (WB API заблокирован на DNS)."),
        ("", "Скоринг на уровне бренда — точность ~±30%."),
        ("", "Для точного матча запустить wb_product_indexer.py --method oem через VPN."),
    ]
    ws6.column_dimensions['A'].width = 30
    ws6.column_dimensions['B'].width = 70
    for r_data in notes:
        row_idx = ws6.max_row + 1
        ws6.append(list(r_data))
        if r_data[1] == "":
            ws6.cell(row_idx, 1).font = Font(bold=True, size=11)
...
def main():
    print("=" * 68)
    print("  WB Top-500 v2  |  Avtoliga  |  8 signals  |  Drive2+Autostat")
    print("=" * 68)

    print("\n[1/4] Loading price list...")
    items = load_price()
    print(f"  Articles: {len(items):,}  |  Brands: {len(set(i['brand'] for i in items))}")

    print("\n[2/4] Building brand stats from MPStats cache...")
    bstats = build_brand_stats()
    matched = sum(1 for i in items if i['brand'].upper() in bstats)
    print(f"  Brands in cache: {len(bstats):,}  |  Matched articles: {matched:,}/{len(items):,}")

    print("\n[3/4] Scoring (8 signals)...")
    scored, skipped = [], 0
    for it in items:
    ...
    for it in scored:
        ...
        if brand_cnt[b] < MAX_PER_BRAND:
            brand_cnt[b] += 1
            final.append(it)
    print(f"  Scored: {len(scored):,}  |  Skipped: {skipped}  |  After cap: {len(final):,}")
    ...
    print(f"{'#':>3}  {'Brand':<18} {'Article':<22} {'Buy':>7} {'Sell':>7} "
          f"{'Mg':>5} {'Sls':>5} {'OOS':>4}  {'Score':>7}  Strategies")
    print("-" * 110)
    for i, it in enumerate(final[:20], 1):
        strat_short = it.get('strats','')[:45]
        print(f"{i:>3}  {it['brand']:<18} {it['article']:<22} "
              ...
              f"{it['score']:>7.4f}  {strat_short}")

    # Strategy stats
    print("\n  Strategy breakdown (Top-500):")
    cnt_s = defaultdict(int)
    for it in final[:500]:
        for s in it.get('strats','').split(', '):
            if s.strip(): cnt_s[s.strip()] += 1
    for s, n in sorted(cnt_s.items(), key=lambda x: -x[1])[:15]:
        bar = '#' * min(n // 5, 30)
        try:
            print(f"    {s:<38} {n:>3}  {bar}")
        except UnicodeEncodeError:
            print(f"    {s.encode('ascii','replace').decode():<38} {n:>3}  {bar}")
