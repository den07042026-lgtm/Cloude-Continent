...
fill_params_1406.py
══════════════════════════════════════════════════════
Заполняет «Параметры» через GigaChat для строк, где столбец пустой.

Файл: C:\\Users\\Admin\\Desktop\\Топ ВБ 1306\\Топ-500 ВБ 1406.xlsx
Сохраняет поверх исходника.

Запуск:
  uv run --with playwright,openpyxl fill_params_1406.py
"""
...
BATCH_SIZE    = 5
SAVE_EVERY    = 3
PROMPT_MARKER = "Ты эксперт по техническим характеристикам автозапчастей"
EXAMPLES_COUNT = 3
...
def build_prompt(items: list[dict], examples: list[str]) -> str:
    ...
    instruction = (
        f"{PROMPT_MARKER}. "
        "Для каждой позиции определи технические параметры и характеристики детали.\n\n"
        "Формат: каждый параметр на новой строке в виде «Название: Значение».\n"
        "Включай: тип детали, размеры, материал, конструктивные особенности, OEM-номера если известны, "
        "аналоги популярных брендов.\n"
        "Если точных данных нет — указывай типичные значения для данного вида детали.\n\n"
        f"=== ПРИМЕРЫ ФОРМАТА ===\n{examples_text}\n\n"
        "=== ЗАДАНИЕ ===\n"
        "Верни ТОЛЬКО JSON объект без пояснений и без markdown:\n"
        '{"items": [{"idx": 0, "параметры": "Тип: ...\nРазмер: ..."}, ...]}\n\n'
    )
    ...
    for item in items:
        ...
        if item.get("compat"):
            lines.append(f"Применяемость: {str(item['compat'])[:250]}")
        if item.get("alt"):
            lines.append(f"Аналоги: {str(item['alt'])[:200]}")
        lines.append("")
    return "\n".join(lines)
...
def extract_items(text: str) -> list[dict]:
    ...
    def is_valid(lst):
        return bool(lst) and isinstance(lst[0], dict) and "параметры" in lst[0] and "idx" in lst[0]
...
def wait_for_response(page, timeout_sec: int = 300) -> str:
    ...
    try:
        page.screenshot(path=str(FILE_PATH.parent / "_debug_params_timeout.png"))
    except Exception:
        pass
    print(" таймаут")
    return ""
...
def process(page, row_range=None):
    ...
    if not param_col:
        print("Столбец «Параметры» не найден.")
        wb.close()
        return

    # Примеры из уже заполненных строк
    examples = []
    for r in range(2, ws.max_row + 1):
        v = ws.cell(r, param_col).value
        if v and len(str(v)) > 50:
            examples.append(str(v)[:500])
        if len(examples) >= EXAMPLES_COUNT:
            break

    # Строки с пустыми параметрами
    todos = []
    for r in range(2, ws.max_row + 1):
    ...
    for batch_no, batch_start in enumerate(range(0, total, BATCH_SIZE), start=1):
        ...
        for local_idx, r in enumerate(batch_rows):
            ...
            if not data or not data.get("параметры"):
                continue
            ws.cell(r, param_col).value = str(data["параметры"]).strip()
            filled += 1
...
def main():
    ap = argparse.ArgumentParser(description="Заполнение параметров через GigaChat")
