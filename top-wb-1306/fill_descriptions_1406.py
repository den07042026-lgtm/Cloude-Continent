"""
fill_descriptions_1406.py
══════════════════════════════════════════════════════
Заполняет столбец «Описание» через GigaChat (браузер)
для строк, где описание пустое.

Файл: C:\\Users\\Admin\\Desktop\\Топ ВБ 1306\\Топ-500 ВБ 1406.xlsx
Сохраняет поверх исходника.

Первый запуск / протухла сессия:
  uv run --with playwright,openpyxl fill_descriptions_1406.py --login

Основной запуск:
  uv run --with playwright,openpyxl fill_descriptions_1406.py
"""
...
BATCH_SIZE    = 5
SAVE_EVERY    = 3
PROMPT_MARKER = "Ты профессиональный копирайтер"

EXAMPLES_COUNT = 4   # сколько примеров стиля собирать из файла
...
def build_prompt(items: list[dict], examples: list[str]) -> str:
    ...
    instruction = (
        f"{PROMPT_MARKER} по автозапчастям. "
        "Напиши товарное описание для каждой позиции ниже в том же стиле, "
        "что и примеры: информативно, профессионально, без воды, 3–5 абзацев, "
        "на русском языке. Описывай назначение, конструктивные особенности, "
        "совместимость и преимущества детали.\n\n"
        f"=== ПРИМЕРЫ СТИЛЯ ===\n{examples_text}\n\n"
        "=== ЗАДАНИЕ ===\n"
        "Верни ТОЛЬКО JSON объект без пояснений и без markdown:\n"
        '{"items": [{"idx": 0, "описание": "текст описания"}, ...]}\n\n'
    )
    ...
    for item in items:
        ...
        if item.get("compat"):
            lines.append(f"Применяемость: {str(item['compat'])[:300]}")
        lines.append("")
    return "\n".join(lines)
...
def extract_items(text: str) -> list[dict]:
    ...
    def is_valid(lst):
        return bool(lst) and isinstance(lst[0], dict) and "описание" in lst[0]
...
def find_json_on_page(page) -> str:
    """Ищет JSON с 'описание' прямо в тексте страницы."""
    try:
        return page.evaluate(r"""() => {
            ...
            if (!result) {
                // Ищем массив с "описание"
                const pat = /"описание"\s*:/g;
                let m, lastIdx = -1;
...
def wait_for_response(page, timeout_sec: int = 300) -> str:
    ...
    try:
        page.screenshot(path=str(FILE_PATH.parent / "_debug_desc_timeout.png"))
    except Exception:
        pass
    print(" таймаут")
    return ""
...
def process(page, row_range=None):
    ...
    if not desc_col:
        print("Столбец «Описание» не найден.")
        wb.close()
        return

    # Собираем примеры стиля из уже заполненных строк
    examples = []
    for r in range(2, ws.max_row + 1):
        v = ws.cell(r, desc_col).value
        if v and len(str(v)) > 200:
            examples.append(str(v)[:800])
        if len(examples) >= EXAMPLES_COUNT:
            break

    # Строки с пустым описанием
    todos = []
    for r in range(2, ws.max_row + 1):
    ...
    total = len(todos)
    if not todos:
        print("Нечего обрабатывать — все описания заполнены.")
        wb.close()
        return

    print(f"Строк для заполнения: {total}")
    print(f"Примеров стиля собрано: {len(examples)}")
    ...
    for batch_no, batch_start in enumerate(range(0, total, BATCH_SIZE), start=1):
        ...
        for local_idx, r in enumerate(batch_rows):
            ...
            if not data or not data.get("описание"):
                continue
            ws.cell(r, desc_col).value = str(data["описание"]).strip()
            filled += 1
...
def main():
    ap = argparse.ArgumentParser(description="Заполнение описаний через GigaChat")
    ...
    with sync_playwright() as pw:
        ...
        if args.login:
            print("Открываю браузер GigaChat...")
            try:
        ...
        if any(k in page.url.lower() for k in ("login", "signin", "auth", "sso", "id.sber")):
            print("Не авторизован! Запусти:")
            print("  uv run --with playwright,openpyxl fill_descriptions_1406.py --login")
            context.close()
            sys.exit(1)
